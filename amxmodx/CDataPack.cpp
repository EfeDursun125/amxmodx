...
void CDataPack::Initialize(void)
{
	m_curptr = m_pBase;
	m_size = 0;
}

void CDataPack::CheckSize(const size_t typesize)
{
	if (m_curptr - m_pBase + typesize <= m_capacity)
		return;

	const size_t pos = m_curptr - m_pBase;
	do
	{
		m_capacity *= 2;
...
void CDataPack::ResetSize(void)
{
	m_size = 0;
}

size_t CDataPack::CreateMemory(const size_t size, void **addr)
{
...
void CDataPack::PackCell(const cell cells)
{
...
void CDataPack::PackString(const char *string)
{
	...
	const size_t maxsize = sizeof(char) + sizeof(size_t) + len + 1;
	CheckSize(maxsize);
...
void CDataPack::Reset(void) const
{
	m_curptr = m_pBase;
}

size_t CDataPack::GetPosition(void) const
{
	return static_cast<size_t>(m_curptr - m_pBase);
}

bool CDataPack::SetPosition(const size_t pos) const
{
	if (pos > m_size-1)
		return false;
...
bool CDataPack::CanReadCell(void) const
{
...
cell CDataPack::ReadCell(void) const
{
	if (!CanReadCell())
		return 0;
	...
	m_curptr += sizeof(char);
	m_curptr += sizeof(size_t);
	const cell val = *reinterpret_cast<cell*>(m_curptr);
	m_curptr += sizeof(cell);
	return val;
}

bool CDataPack::CanReadFloat(void) const
{
...
float CDataPack::ReadFloat(void) const
{
	if (!CanReadFloat())
		return 0;
	...
	m_curptr += sizeof(char);
	m_curptr += sizeof(size_t);
	const float val = *reinterpret_cast<float*>(m_curptr);
	m_curptr += sizeof(float);
	return val;
}
...
void *CDataPack::GetMemory(void) const
{
	return m_curptr;
}
...
bool CDataPack::CanReadMemory(size_t *size) const
{
	...
	const size_t bytecount = *(size_t*)(m_curptr + sizeof(char));
