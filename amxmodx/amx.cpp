...
{
    if (amxmodx_perflog->value > 0.01f)
    {
        CPluginMngr::CPlugin* perf_Plug = g_plugins.findPluginFast(amx);
        if (perf_Plug && (perf_Plug->isDebug() || static_cast<int>(amxmodx_debug->value) == 2))
        {
            ...
            const float ms_float = static_cast<float>(duration_cast<microseconds>(steady_clock::now() - t1).count() / 1000.0f);
