...
extern float clogf(const float value);

// https://martin.ankerl.com/2012/01/25/optimized-approximative-pow-in-c-and-cpp/
inline double cpow(const double a, const double b)
{
	union
	{
		double d;
	...
	u.x = static_cast<int>(b * (u.x - 1072632447) + 1072632447);
	return u.d;
}
...
inline int8_t cabsi8(const int8_t value)
{
...
private:
	...
	uint16_t m_capacity;
public:
	MiniArray(const uint16_t size = 0) : m_size(size), m_capacity(size) { safeloc(m_array, size); }
