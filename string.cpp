...
char* format_amxstring(AMX *amx, cell *params, const int parm, int& len)
{
...
template <typename T>
int set_amxstring_utf8(AMX *amx, cell amx_addr, const T *source, const size_t sourcelen, size_t maxlen)
{
...
char *get_amxbuffer(const int id)
{
...
{
	char* buffer = get_amxbuffer(id);
	get_amxstring_r(amx, amx_addr, buffer, MAX_BUFFER_LENGTH - 1);
	return buffer;
}
...
char *get_amxstring_null(AMX *amx, const cell amx_addr, const int id, int& len)
{
	...
	if (get_amxaddr(amx, amx_addr) == plugin->getNullStringOfs())
		return nullptr;
...
cell *get_amxvector_null(AMX *amx, cell amx_addr)
{
	...
	if (addr == plugin->getNullVectorOfs())
		return nullptr;
